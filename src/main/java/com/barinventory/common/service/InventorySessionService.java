...
public class InventorySessionService {
	...
	public InventorySession createSession(Long barId) {
		...
		InventorySession session = InventorySession.builder().bar(bar).status(SessionStatus.IN_PROGRESS)
				.sessionStartTime(LocalDateTime.now()).build();
	...
	public Optional<InventorySession> getPreviousSession(Long barId) {
		List<InventorySession> completedSessions = sessionRepository.findCompletedSessionsByBar(barId);
	...
	/**
	 * Parse and save stockroom form data Form pattern: brandSize_[id]_opening,
	 * brandSize_[id]_received, etc.
	 */
	@Transactional
	public void saveStockroomFromForm(Long sessionId, Map<String, String> formData) {
		...
		for (Map.Entry<Long, Map<String, String>> entry : groupedData.entrySet()) {
			...
			com.barinventory.brands.entity.BrandSize brandSize = brandSizeRepository.findByIdAndActiveTrue(brandSizeId)
			...
			StockroomInventory stockroom = StockroomInventory.builder().session(session).brandSize(brandSize)
					.openingStock(opening).receivedStock(received).closingStock(closing).transferredOut(transferred)
					.build();
	...
	/**
	 * Create or update distribution records based on stockroom transferred
	 * quantities
	 */
	@Transactional
	public void createOrUpdateDistribution(Long sessionId) {
		...
		for (StockroomInventory stockroom : stockrooms) {
			...
			int updated = distributionRepository.updateDistributionBulk(sessionId, brandSizeId, transferred);

			// If no record exists, create new one
			if (updated == 0) {
				DistributionRecord dr = DistributionRecord.builder().session(session)
						.brandSize(stockroom.getBrandSize()).quantityFromStockroom(transferred)
						.totalAllocated(BigDecimal.ZERO).unallocated(transferred)
						.status(DistributionStatus.PENDING_ALLOCATION).build();
	...
	/**
	 * Parse and save distribution allocations Form pattern:
	 * dist_[brandSizeId]_well_[wellId]=[quantity]
	 */
	@Transactional
	public void saveDistributionAllocations(Long sessionId, Map<String, String> formData) {
				...
				if (parts.length >= 4) {
					try {
						...
						if (qty.compareTo(BigDecimal.ZERO) > 0) {
							allocations.computeIfAbsent(brandSizeId, k -> new HashMap<>()).put(wellId, qty);
						}
		...
		for (Map.Entry<Long, Map<Long, BigDecimal>> entry : allocations.entrySet()) {
			...
			BigDecimal totalQty = wellAllocations.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);

			DistributionRecord dr = distributionRepository.findBySessionSessionIdAndBrandSizeId(sessionId, brandSizeId)
					.orElseThrow(() -> new RuntimeException("Distribution not found for brandSize: " + brandSizeId));
	...
	public Map<Long, BigDecimal> getDistributionMapForSession(Long sessionId) {
		return wellRepository.findBySessionSessionId(sessionId).stream()
				...
				.collect(Collectors.groupingBy(w -> w.getBrandSize().getId(),
						Collectors.mapping(WellInventory::getReceivedFromDistribution,
	...
	/**
	 * Parse and save well inventory form data Form pattern: well_[wellId]_opening,
	 * well_[wellId]_received, etc.
	 */
	@Transactional
	public void saveWellInventoryFromForm(Long sessionId, Map<String, String> formData) {
		...
		for (BrandSize brandSize : brandSizes) {
			for (BarWell well : wells) {
				...
				if (opening.compareTo(BigDecimal.ZERO) == 0 && received.compareTo(BigDecimal.ZERO) == 0
						&& closing.compareTo(BigDecimal.ZERO) == 0) {
					continue;
				}

				WellInventory wellInv = WellInventory.builder().session(session).brandSize(brandSize).barWell(well)
						.openingStock(opening).receivedFromDistribution(received).closingStock(closing).build();
	...
	public List<String> getWellNamesForBar(Long barId) {
		return barWellRepository.findByBarBarIdAndActiveTrue(barId).stream().map(BarWell::getWellName)
	...
	public BigDecimal getPreviousClosingForStockroom(Long barId) {
		...
		return stockroomRepository.findBySessionSessionId(previousSession.get().getSessionId()).stream()
				.map(s -> safe(s.getClosingStock())).reduce(BigDecimal.ZERO, BigDecimal::add);
	}
	...
	public BigDecimal getPreviousClosingForWells(Long barId) {
		...
		return wellRepository.findBySessionSessionId(previousSession.get().getSessionId()).stream()
				.map(w -> safe(w.getClosingStock())).reduce(BigDecimal.ZERO, BigDecimal::add);
	}

	/**
	 * Get product-wise summary for a bar within date range
	 */
	public Map<String, Object> getProductWiseSummaryOptimized(Long barId, LocalDateTime startDate,
	...
	public boolean validateStockroomToDistribution(Long sessionId, StringBuilder errors) {
		...
		for (StockroomInventory stockroom : stockrooms) {
			...
			DistributionRecord distribution = distributions.stream()
					.filter(d -> d.getBrandSize().getId().equals(stockroom.getBrandSize().getId())).findFirst()
					.orElse(null);

			if (distribution == null && transferred.compareTo(BigDecimal.ZERO) > 0) {
				errors.append("BrandSize ").append(stockroom.getBrandSize().getId()).append(": Missing distribution. ");
				return false;
			}

			if (distribution != null && transferred.compareTo(distribution.getQuantityFromStockroom()) != 0) {
				errors.append("BrandSize ").append(stockroom.getBrandSize().getId()).append(": quantity mismatch. ");
				return false;
			}
		}
	...
	public boolean validateDistributionToWells(Long sessionId, StringBuilder errors) {
		...
		for (DistributionRecord dr : distributions) {
			BigDecimal wellsTotal = wellRepository.sumReceivedBySessionAndBrandSize(sessionId,
					dr.getBrandSize().getId());

			if (safe(dr.getTotalAllocated()).compareTo(safe(wellsTotal)) != 0) {
				errors.append("BrandSize ").append(dr.getBrandSize().getId()).append(": allocation mismatch. ");
				return false;
			}
		}
	...
	public boolean validateNoUnallocatedStock(Long sessionId, StringBuilder errors) {
		...
		for (DistributionRecord dr : distributions) {
			if (safe(dr.getUnallocated()).compareTo(BigDecimal.ZERO) > 0) {
				errors.append("BrandSize ").append(dr.getBrandSize().getId()).append(": unallocated stock detected. ");
				return false;
			}
		}
	...
	public InventorySession createSetupSession(Long barId) {
		...
		InventorySession session = InventorySession.builder().bar(bar).status(SessionStatus.SETUP)
				.sessionStartTime(LocalDateTime.now()).build();
	...
	public List<BrandSize> getAllActiveBrandSizes() {
		return brandSizeRepository.findAll().stream().filter(BrandSize::isActive).collect(Collectors.toList());
	}
	...
	public void saveSetupStockroom(Long sessionId, Map<String, String> formData) {
		...
		for (BrandSize brandSize : brandSizes) {
			...
			if (openingQty.compareTo(BigDecimal.ZERO) > 0) {
				StockroomInventory si = StockroomInventory.builder().session(session).brandSize(brandSize)
						.openingStock(openingQty).receivedStock(BigDecimal.ZERO).closingStock(openingQty)
						.transferredOut(BigDecimal.ZERO).remarks("Admin initial setup").build();
	...
	public void saveSetupWells(Long sessionId, Map<String, String> formData) {
		...
		for (BarWell well : wells) {
			for (BrandSize brandSize : brandSizes) {
				...
				if (openingQty.compareTo(BigDecimal.ZERO) > 0) {
					WellInventory wi = WellInventory.builder().session(session).brandSize(brandSize).barWell(well)
							.openingStock(openingQty).receivedFromDistribution(BigDecimal.ZERO).closingStock(openingQty)
							.consumed(BigDecimal.ZERO).remarks("Admin initial setup").build();
	...
	public Map<Long, BigDecimal> getSetupStockroomData(Long sessionId) {
		return stockroomRepository.findBySessionSessionId(sessionId).stream().collect(
				Collectors.toMap(s -> s.getBrandSize().getId(), StockroomInventory::getClosingStock, (a, b) -> a));
	}

	public Map<String, BigDecimal> getSetupWellsData(Long sessionId) {
		return wellRepository.findBySessionSessionId(sessionId).stream()
				.collect(Collectors.toMap(w -> w.getBrandSize().getId() + "_" + w.getBarWell().getBarWellId(),
						WellInventory::getClosingStock, (a, b) -> a));
	}
	...
	/**
	 * Group form data by key prefix e.g., groupFormDataByKey(data, "brandSize")
	 * groups brandSize_1_opening, brandSize_1_received
	 */
			...
			if (key.startsWith(prefix + "_")) {
				...
				if (parts.length >= 3) {
					try {
						...
						grouped.computeIfAbsent(id, k -> new HashMap<>()).put(field, value);
					} catch (NumberFormatException e) {
		...
		for (Map.Entry<String, String> entry : formData.entrySet()) {
			...
			if (key.startsWith("qty_") && value != null && !value.isBlank()) {
				...
				if (qty.compareTo(BigDecimal.ZERO) > 0) {
					...
					BrandSize brandSize = brandSizeRepository.findById(brandSizeId).orElseThrow();
	...
	public void preloadOpeningStock(Long sessionId) {
		...
		List<StockroomInventory> openingStock = stockroomRepository.findByBar_BarIdAndOpeningTrue(barId);
	...
	// In com.barinventory.common.service.InventorySessionService
	// In InventorySessionService.java

	@Transactional(readOnly = true)
	public List<InventorySession> getRecentSessions(Long barId, int limit) {
	    return sessionRepository
	        .findByBar_BarIdOrderBySessionStartTimeDesc(barId)
	        .stream()
	        .limit(limit)
	        .collect(Collectors.toList());
	}
